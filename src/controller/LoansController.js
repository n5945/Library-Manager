...
class LoansController {
    ...
    async borrowBooks(req, res) {
        ...
        return res.status(201).json("Emprestimo realizado com sucesso.")
    ...
    async listBorrowedBooks(req, res) {
        ...
        const loans = await knex("loans").where({user_id}).innerJoin('books','books.id', 'loans.book_id',).select('books.title', 'books.author', 'books.pages', 'books.category')
    ...
    async returnBorrowedBooks(req, res) {
        ...
        if(bookId == book_id) {
            await knex("books").where({id: book_id}).update({availability: true})
